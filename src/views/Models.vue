...
const camera = new PerspectiveCamera(80, size.aspect(), 0.1, 1000);
...
const pointLight = new PointLight("#ffffff", 2);
pointLight.position.z = 100;
pointLight.castShadow = true;

const plane = new Mesh(
  new PlaneGeometry(10, 10),
  new MeshStandardMaterial({ color: new Color("gray"), roughness: 0.3 })
);
...
gltfLoader.load(build, (house) => {
  house.scene.scale.set(0.05, 0.05, 0.05);
  house.scene.children[0].castShadow = true;
...
scene.add(camera, amLight, directLight, pointLight);
